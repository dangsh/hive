...
def login(num):
    with open('qq.json' , encoding='utf-8') as f:
        lines = f.readlines()
        line = lines[num:]
        d = json.loads(line[0])
        try:
...
for i in range(3):
    ...
    time.sleep(5)
    try:
        b.find_link_by_href("https://passport.weibo.cn/signin/other?r=http%3A%2F%2Fm.weibo.cn%2F").click()
    except:
        print("找不到第三方账号按钮")
    time.sleep(2)

    try:
        b.find_link_by_href("https://passport.weibo.com/othersitebind/authorize?entry=mweibo&site=qq&res=other&callback=http%3A%2F%2Fm.weibo.cn%2F").click()
    except:
        print("找不到qq账号登录")
    time.sleep(4)
