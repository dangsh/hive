...
import os
import time
tmp = os.path.exists("data/lagou.csv")
c = open("data/lagou.csv", 'a')
...
for news in soup.select('.container-body .dn dl'):
    ...
    for i, item in enumerate(news.select('a')):
            ...
            tmp2 = os.path.exists("data2/lagou2_" + str(i+1) + ".csv")
            c2 = open("data2/lagou2_" + str(i+1) + ".csv", 'a')
            ...
            ipage = 0
            if len(soup2.select('.pager_container')) >= 1:
                maxPage = soup2.select('.pager_container')[0].select('a')[4].text
            else:
                ipage += 1
                print("Get page", ipage)
                res2 = requests.get(categoryURL + "1", headers = headers)
                soup2 = BeautifulSoup(res2.text, 'html.parser')
            ...
            for j in range(int(maxPage)):
                ...
                for news in soup2.select('.item_con_list li'):
                    try:
                        ...
                        tim = "null"
                        city = "null"
                        if len(news.select('.position_link')) >= 1:
                        ...
                        if len(news.select('.format-time')) >= 1:
                            tim = news.select('.format-time')[0].text.strip()
                        if len(news.select('em')) >= 1:
                            city = news.select('em')[0].text.strip()
                        writer2.writerow([positionName, company, salary, city, tim, url])
                        
                        
                        tmp3 = os.path.exists("data3/lagou3_" + str(i + 1) + ".csv")
                        c3 = open("data3/lagou3_" + str(i + 1) + ".csv", 'a')
                        ...
                        companyName = "null"
                        companyField = "null"
                        companyDevelopmentStage = "null"
                        companySize = "null"
                        companyURL = "null"
                        jobAdvantage = "null"
                        jobDescription = "null"

                        if len(soup3.select('.position-content-l .company')) >= 1:
                            company = soup3.select('.position-content-l .company')[0].text.strip()
                        if len(soup3.select('.job-name .name')) >= 1:
                            jobName = soup3.select('.job-name .name')[0].text.strip()
                        if len(soup3.select('.job_request span')) >= 5:
                        ...
                        if len(soup3.select('.job-advantage p')) >= 1:
                        ...
                        if len(soup3.select('.b2')) >= 1:
                            companyName = soup3.select('.b2')[0].attrs['alt']
                        if len(soup3.select('.c_feature li')) >= 3:
                        ...
                        if len(soup3.select('.c_feature a')) >= 1:
                            companyURL = soup3.select('.c_feature a')[0].text.strip()
                        if len(soup3.select('.work_addr')) >= 1:
